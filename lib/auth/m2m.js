  ...
  return {
    ...
    getMachineToken: (clientId, clientSecret) => {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (appCachedToken) {
          let decodedAppCachedToken = jwt.decode(appCachedToken)
          if (decodedAppCachedToken.exp && (decodedAppCachedToken.exp * 1000) <= (new Date().getTime())) {
            appCachedTokenExpired = true
          }
        }

        if (appCachedToken && !appCachedTokenExpired && (new Date().getTime() - cachedTime[clientId]) < tokenCacheTime) {
          ...
          request.post(options, function (error, response, body) {
            ...
            if (body.expires_in) {
              ...
              tokenCacheTime = (body.expires_in - 60) * 1000   // minus 60 seconds  
            }
