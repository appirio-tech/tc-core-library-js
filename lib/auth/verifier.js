            ...
            if (validIssuers.indexOf(decodedToken.payload.iss) === -1){
              ...
              if (!jwksClients[decodedToken.payload.iss]){
                jwksClients[decodedToken.payload.iss] = jwksRSA({
                  ...
                  jwksUri: decodedToken.payload.iss + '.well-known/jwks.json' // TODO I just found out that "issuer URL" + ".well-known/jwks.json" is a conention but not a guarenteed standard
                })
              }
              jwksClients[decodedToken.payload.iss].getSigningKey(kid, (err, key) => {
                if (err) {
                  ...
                  jwtVerify(token, signingKey, callback)
                }
              })
            }
        }
